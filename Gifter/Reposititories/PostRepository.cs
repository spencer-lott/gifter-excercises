    ...
    public class PostRepository : BaseRepository, IPostRepository
    {
        ...
        public Post GetPostByIdWithComments(int id)
        {
            using (var conn = Connection)
            {
                ...
                {
                    cmd.CommandText = @"
              SELECT p.Title, p.Caption, p.DateCreated, p.ImageUrl, p.UserProfileId, u.Id AS UId, u.[Name], u.Email, u.ImageUrl, u.DateCreated, c.Message, c.Id AS CommentId, c.PostId AS CPId, c.UserProfileId AS CUId
                FROM Post p
                LEFT JOIN UserProfile u ON u.Id = p.UserProfileId
                LEFT JOIN Comment c ON c.PostId = p.Id
              WHERE p.Id = @id";
                    ...
                    while (reader.Read())
                    {
                        ...
                        Comment comment = new Comment
                        {
                            Id = DbUtils.GetInt(reader, "CommentId"),
                            Message = DbUtils.GetString(reader, "Message"),
                            PostId = DbUtils.GetInt(reader, "CPId"),
                            UserProfileId = DbUtils.GetInt(reader, "CUId")
                        };
